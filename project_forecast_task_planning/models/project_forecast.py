...
class PlanningShift(models.Model):
    ...
    forecast_hours = fields.Float("Forecast Hours", compute='_compute_forecast_hours',
                                  help="Number of hours already forecast for this task (and its sub-tasks).")
    parent_id = fields.Many2one(
        'project.task', related='task_id.parent_id', store=True)  # store for group by
    resource_id = fields.Many2one(
        compute='_compute_resource_id', store=True, readonly=False)

    _sql_constraints = [
        ('project_required_if_task', "CHECK( (task_id IS NOT NULL AND project_id IS NOT NULL) OR (task_id IS NULL) )",
            ...
            if not task_sudo:
                ...
                if slot._origin.task_id == task_sudo:
                    slot.forecast_hours = task_sudo.forecast_hours - \
                        slot._origin.allocated_hours + slot.allocated_hours
                else:
        ...
        for forecast in self:
            if forecast.task_id and (forecast.task_id not in forecast.project_id.with_context(active_test=False).tasks):
                raise ValidationError(
                    _("Your task is not in the selected project."))
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('task_id'):
                vals['project_id'] = self.env['project.task'].browse(
                    vals.get('task_id')).project_id.id
        return super().create(vals_list)

    def write(self, values):
        if 'task_id' in values and values['task_id'] and 'project_id' not in values:
            values['project_id'] = self.env['project.task'].browse(
