...
class PlanningShift(models.Model):
    ...
    task_id = fields.Many2one(
        ...
        copy=True,
        check_company=True,
        domain="[('project_id', '=?', project_id),('allow_forecast', '=', True)]",
    )

    planned_hours = fields.Float(
        "Initially Planned Hours", related="task_id.planned_hours"
    )
    forecast_hours = fields.Float(
        "Forecast Hours",
        compute="_compute_forecast_hours",
        help="Number of hours already forecast for this task (and its sub-tasks).",
    )
    parent_id = fields.Many2one(
        "project.task", related="task_id.parent_id", store=True
    )  # store for group by
        ...
        for slot in self:
            ...
            if not task_sudo:
                ...
                if slot._origin.task_id == task_sudo:
                    slot.forecast_hours = (
                        task_sudo.forecast_hours
                        - slot._origin.allocated_hours
                        + slot.allocated_hours
                    )
                else:
                    slot.forecast_hours = (
                        task_sudo.forecast_hours + slot.allocated_hours
                    )
            else:
    ...
    def _compute_project_id(self):
        for slot in self:
            ...
            if slot.template_id:
                ...
                and slot.previous_template_id.project_id == slot.project_id
            ):
    ...
    def _check_task_in_project(self):
        for forecast in self:
            if forecast.task_id and (
                forecast.task_id
                not in forecast.project_id.with_context(active_test=False).tasks
            ):
    ...
    def _prepare_template_values(self):
        ...
        return {"task_id": self.task_id.id, **result}
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("task_id"):
                vals["project_id"] = (
                    self.env["project.task"].browse(vals.get("task_id")).project_id.id
                )
    ...
    def write(self, values):
        if "task_id" in values and values["task_id"] and "project_id" not in values:
            values["project_id"] = (
                self.env["project.task"].browse(values["task_id"]).project_id.id
            )
